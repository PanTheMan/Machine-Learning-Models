...
#			self.assignments.append(value)
	...
	def Restore(self, possibilities):
		...
		print("Restored",self.row,self.col, "to",self.possibilities)
		print("Assignments Tried", self.assignments)

	# When possibilties is 0, backtrack instead of soft resetting
	def Backtrack(self, possibilities):
		self.value = 0
		print("HARDRESET", self.row,self.col,self.possibilities,self.assignments,"RESSETING TO", possibilities)
		self.possibilities = self.possibilities + self.assignments
		self.assignments = []
	...
	def ForwardCheck(self,square):
		...
		for sq in AffectedSquares:
			if square.value in sq.possibilities and sq.value == 0 and len(sq.possibilities)==1:
				print("Forward Check Failed due to",sq.row,sq.col, sq.possibilities,sq.assignments)
				return False

		for test in AffectedSquares:
			if square.value in test.possibilities and sq.value == 0:
...
def ConstructBoard(fileName):
	...
	for i in range(9):
		...
		for j in range(9):
			board.SetupSq(i,j,int(rowData[j]))
			if int(rowData[j]) == 0:
